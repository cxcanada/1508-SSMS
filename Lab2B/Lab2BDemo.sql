...
select firstname + ' ' + lastname 'Name', phone from client
where clientid = 4

--b.For all clients, select the client first name, last name and the number of jobs they have. (3 marks)
select firstname, lastname, count(jobnumber) 'Count' from client
...
select avg(costperhour) 'AverageCostPerHour' from service
...
select supply.supplycode, description from supply
left outer join jobsupply on supply.supplycode = jobsupply.supplycode
group by supply.supplycode, description
having count(jobnumber) > 2
...
where supplycode not in (select supplycode from jobsupply)
...
group by staff.staffid, firstname, lastname
having max(total) >= all (select max(total) from job where month(date) = month(getDate()))

--k.How many supplies are there in each supply category? Select the supplycategorycode, description and the count.(3 marks)
Select  supplycategory.supplycategorycode, supplycategory.description, count(supplycode) 'Count' from supply
right outer join supplycategory on supplycategory.supplycategorycode = supply.supplycategorycode
...
values ('Siding10', 'Basic Siding', 45)
go
insert into service(serviceCode, Description, CostPerHour)
values ('Scraping15', 'Scraping', 15)

--2
go
update service
set 
CostPerHour = CostPerHour * 1.1
