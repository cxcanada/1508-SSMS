...
select firstname + ' ' + lastname 'ClientName', phone from client
where clientid = 4

--b.For all clients, select the client first name, last name and the number of jobs they have. (3 marks)
select firstname, lastname, count(*) 'JobCount' from client
...
select supply.supplycode, description, count(*) from supply
inner join jobsupply on supply.supplycode = jobsupply.supplycode
...
group by firstname, lastname
having max(SubTotal) >= ALL(select max(subtotal) from job)

--k.How many supplies are there in each supply category? Select the supplycategorycode, description and the count.(3 marks)
select supplycategory.SupplyCategorycode, supplycategory.description, count(supply.SupplyCode) 'SupplyCount' from Supply
