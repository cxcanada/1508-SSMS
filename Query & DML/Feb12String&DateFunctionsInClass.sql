...
select substring (coursename, 3, 7) 'coursename', coursename from course
...
select year(getdate())
select * from staff
select firstname + ' ' + lastname 'FullName', datehired, datename(mm,datehired) 'hiredmonth' from staff
order by datepart(mm, datehired) asc
...
select firstname + ' ' + lastname 'FullName', datehired, datename(mm, datehired) 'MonthHired' from staff
